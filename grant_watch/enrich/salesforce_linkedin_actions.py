...
class LinkedInPersonDraft:
    ...
    def research_note(self, action_id: str, requester: str) -> str:
        ...
        if self.organization.source_url:
            lines.append(
                f"Official organization source: {self.organization.source_url}"
            )
        lines.extend(
            (
...
def _draft(
    conn: sqlite3.Connection, candidate: linkedin_candidates.LinkedInCandidate
) -> LinkedInPersonDraft:
    """Join a candidate to its verified funding lead and available official profile."""
    row = db.get_lead(conn, candidate.lead_id)
    if (
        row is None
        or str(row["entity_name"] or "").casefold() != candidate.organization.casefold()
    ):
    ...
    if (
        str(row["current_event_verification_status"] or "") != "verified"
        or not funding_url
    ):
...
def _matching_placeholder(
    ...
    if len(lead_ids) > 1:
        raise ValueError(
            "Salesforce has multiple matching Leads; no record was selected"
        )
    if not lead_ids:
        if matches:
            links = ", ".join(item.link for item in matches[:3])
            raise ValueError(
                f"Salesforce already has a possible matching record: {links}"
            )
        return ""
    ...
    placeholder = (
        ...
        and not any(
            (snapshot.first_name, snapshot.email, snapshot.title, snapshot.linkedin_url)
        )
    ...
    if (
        ledger is None
        or str(ledger["state"]) != "complete"
        or str(ledger["item_state"]) != "lead_created"
        or str(ledger["salesforce_id"] or "") != snapshot.record_id
    ):
        raise ValueError(
            "The existing Grant Lead must be reconciled before it is updated"
        )
...
def prepare_linkedin_person(
    ...
    if placeholder_id:
        ...
        delta = {
            ...
            if key in mutable
            or (key in enrichment.values and enrichment.values[key] in (None, ""))
        }
    ...
    plan = workflow.MemberPlan(
        draft.grant_lead_id,
        f"linkedin:{candidate_id}",
        draft.company,
        draft.state,
        operation,
        proposed_lead=desired,
        note=json.dumps(
            {"candidate_id": candidate_id, "profile_url": draft.profile_url}
        ),
    )
...
def confirm_linkedin_person(
    ...
    if row["action_type"] == "create_linkedin_person_lead":
        ...
        if duplicate_organization(draft.company, draft.state):
            raise ValueError(
                "Salesforce changed after preview; no duplicate Lead was created"
            )
        ...
        if not result.success or not result.lead_id:
            raise SalesforceCompositeRolledBack(
                result.error or "Salesforce rolled back"
            )
        ...
        if (
            before.company.casefold() != draft.company.casefold()
            or before.system_modstamp != str(payload["system_modstamp"])
            or before.last_name.casefold() != draft.company.casefold()
            or any((before.first_name, before.email, before.title, before.linkedin_url))
        ):
            raise ValueError("Salesforce Lead changed after preview")
        others = [
            item
            for item in gateway.exact_linkedin_person_leads(
                draft.profile_url, draft.company, str(fields["LastName"])
            )
    ...
    if not _verify_identity(after, draft.company, fields):
        raise ValueError(
            "Salesforce Lead person fields did not match the approved preview"
        )
    ...
    return workflow.ActionExecution(
        ...
        f"Added {draft.name} to Salesforce: {after.link}",
        added=1,
    )
