...
def _candidates(conn: sqlite3.Connection, limit: int) -> list[sqlite3.Row]:
    ...
    rows.sort(key=lambda row: scoring.lead_score(
        str(row["program"] or ""), row["amount"], str(row["event_date"] or "")),
              reverse=True)
    return rows[:max(1, min(limit, 100))]


def _persist(conn: sqlite3.Connection, lead_id: int,
             result: salesforce.SFResult) -> None:
    ...
    with conn:
        ...
        conn.executemany(
            """INSERT INTO salesforce_matches
               ...
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
            [(lead_id, match.sobject, match.record_id, match.name,
              match.company or None, match.owner or None, match.link,
              match.confidence, match.account_id or None, match.stage or None,
...
def sync(conn: sqlite3.Connection, limit: int = 25,
         dry_run: bool = False) -> SyncSummary:
