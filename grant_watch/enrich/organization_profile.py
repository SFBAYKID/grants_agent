...
def _verified_text(page: str, value: object) -> str:
    ...
    return candidate if candidate and _normalized(candidate) in _normalized(page) else ""
...
def extract_profile(page: str, official_domain: str, source_url: str,
...
def fetch_profile(entity: str, official_domain: str,
                  contact_source_url: str) -> OrganizationProfile:
    ...
    pages = [text for url in urls if _host(url) == _host(root)
             for text in [_scrape(url)] if text]
    if not pages:
    ...
    response = Anthropic().messages.create(
        model=os.environ.get("GRANT_MODEL", "claude-sonnet-5"), max_tokens=500,
        messages=[{"role": "user", "content": prompt}])
    raw = "".join(str(block.text) for block in response.content
