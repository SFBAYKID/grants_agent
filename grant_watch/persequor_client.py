...
def _angle(row: sqlite3.Row) -> str:
    ...
    if source == "grants.gov":
        return "published funding opportunity; application dates are in the source record"
    if grade == "silver":
...
def build_brief(row: sqlite3.Row, contact: sqlite3.Row | None,
                ...
                request_id: str | None = None) -> OutreachBrief | None:
...
def submit_brief(conn: sqlite3.Connection, lead_id: int,
                 brief: OutreachBrief) -> tuple[str, str]:
    ...
    saved = conn.execute(
        "SELECT * FROM outreach WHERE request_id=?", (request_id,)).fetchone()
    assert saved is not None
    if saved["status"] == "submitted":
        return "submitted", "This request was already accepted by Persequor."
    if saved["status"] == "rejected":
        return "rejected", "Persequor previously rejected this request; it was not retried."
    if inserted.rowcount == 0:
        if saved["status"] in {"queued", "sending"}:
            return "unreachable", ("This exact request is already queued or being sent to "
                                   "Persequor; Grant did not create another copy.")
        if saved["status"] == "failed":
            return "rejected", ("This request reached its retry limit; Grant did not "
                                "create another copy.")
    return _attempt_saved(conn, saved)


def _attempt_saved(conn: sqlite3.Connection,
                   outreach: sqlite3.Row) -> tuple[str, str]:
    ...
    if claimed.rowcount != 1:
        return "unreachable", ("This request is already being processed; Grant did not "
                               "send another copy.")
...
def _queue_retry(conn: sqlite3.Connection, outreach_id: int, attempt: int,
                 error: str) -> None:
...
def retry_pending(conn: sqlite3.Connection, dry_run: bool = False,
                  limit: int = 20) -> RetrySummary:
