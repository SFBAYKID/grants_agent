...
def _deadline(raw: str) -> str:
    ...
    try:
        return datetime.strptime(raw.strip(), "%b %d, %Y %I:%M:%S %p").date().isoformat()
    except ValueError:
        return ""


def parse_table_rows(rows: list[list[object]],
    ...
    for cells in rows:
        ...
        if not _PHYSICAL_RE.search(description) or _CYBER_RE.search(description):
            continue
        detail_url = (
            f"{DETAIL_URL}?docId={bid_id}&external=true&parentUrl=close")
        out.append(RawItem(
            source="oregonbuys",
            item_id=bid_id,
