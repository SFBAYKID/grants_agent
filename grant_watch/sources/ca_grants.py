    ...
    for row in csv.DictReader(io.StringIO(normalized)):
        ...
        out.append(RawItem(
            ...
            raw={key: row.get(key) for key in (
                "PortalID", "Status", "LastUpdated", "ApplicantType", "Geography",
                "EstAvailFunds", "EstAmounts", "AwardStats")},
            ...
            eligible_scope=(row.get("ApplicantTypeNotes") or row.get("ApplicantType") or "")[:500],
...
def parse_awards(csv_text: str, fiscal_year: str, source_url: str,
                 today: date | None = None) -> list[RawItem]:
    """Parse approved physical-security awards without treating publish date as award date."""
    today = today or date.today()
    out: list[RawItem] = []
    normalized = csv_text.removeprefix("\ufeff").removeprefix("ï»¿")
    for row in csv.DictReader(io.StringIO(normalized)):
        notes = " ".join((row.get("AwardAmountNotes") or "",
                          row.get("AwardCancellingNotes") or ""))
        ...
        if not entity or not _physical_security(evidence):
            continue
        if ("nonprofit security grant" not in evidence.lower()
        ...
        out.append(RawItem(
            ...
            url=source_url,
            raw={key: row.get(key) for key in (
                "PortalID", "GrantID", "PublishDate", "LastUpdated", "FiscalYear",
                "RecipientType", "ProjectStatus", "CountiesServed")},
...
def _csv_resource_url(package_id: str) -> str:
    ...
    payload = cast(dict[str, object], polite_get(
        CKAN_PACKAGE_API, params={"id": package_id}).json())
