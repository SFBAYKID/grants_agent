...
def watch_states() -> tuple[str, ...]:
    ...
    if not raw:
        return ALL_STATES
    states = tuple(dict.fromkeys(part.strip().upper() for part in raw.split(",")
                                 if part.strip()))
...
def _query_page(cfda: str, state: str, page: int,
                subawards: bool = False) -> dict[str, Any]:
...
def parse_awards(payload: dict[str, Any], cfda: str, state: str) -> list[RawItem]:
    ...
    for a in payload.get("results", []):
        ...
        out.append(RawItem(
            ...
            backfill=bool(a.get("Start Date") and str(a["Start Date"])[:10]
                          < (date.today() - timedelta(days=90)).isoformat()),
        ))
    return out


def parse_nsgp_subawards(payload: dict[str, Any], state: str,
    ...
    for award in payload.get("results", []):
        ...
        out.append(RawItem(
            ...
            url=(f"https://www.usaspending.gov/award/{parent_id}"
                 if parent_id else ""),
            raw={key: award.get(key) for key in (
                "Sub-Award ID", "Sub-Award Amount", "Sub-Award Date",
                "prime_award_generated_internal_id")},
            event_type=FundingEventType.AWARD_OBLIGATED,
            event_date=occurred,
            ...
            verification_status=VerificationStatus.VERIFIED,
            backfill=(not occurred or occurred < (today - timedelta(days=90)).isoformat()),
        ))
    return out
