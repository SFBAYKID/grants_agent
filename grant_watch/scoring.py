...
AWARD_SOURCES_PREFIX = ("usaspending:", "usaspending-subaward:",
                        "ca-grants-award:", "seed:")
...
def grade(item: RawItem, today: date | None = None) -> Lead:
    ...
    if item.source in RFP_SOURCES:
        ...
        return Lead(item, LeadGrade.SILVER if deadline and deadline >= today
                    else LeadGrade.WATCH)
...
def is_fresh(item: RawItem, today: date | None = None) -> bool:
    ...
    return occurred is not None and occurred >= today - timedelta(days=FRESH_MONTHS * 30)
...
def lead_score(program: str, amount: float | None, event_date: str,
