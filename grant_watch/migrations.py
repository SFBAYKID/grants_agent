...
def _migration_3_durable_workflows(conn: sqlite3.Connection) -> None:
    ...
    for definition in (
        "event_id INTEGER", "delivery_key TEXT", "delivery_status TEXT", "urgent INTEGER DEFAULT 0"
    ):
        _add_column(conn, "posts", definition)
    for definition in (
        "request_id TEXT", "status TEXT DEFAULT 'draft'", "attempts INTEGER DEFAULT 0",
        "last_error TEXT", "next_attempt_at TIMESTAMP", "created_at TIMESTAMP",
    ):
        _add_column(conn, "outreach", definition)
    for definition in (
        "official_domain TEXT", "field_evidence_json TEXT"
    ):
...
def _migration_6_release_safety_state(conn: sqlite3.Connection) -> None:
    ...
    _add_column(conn, "crm_actions", "external_write_started INTEGER NOT NULL DEFAULT 0")
...
def _migration_9_salesforce_followup_state(conn: sqlite3.Connection) -> None:
