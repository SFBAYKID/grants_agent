...
def _record_priced_run(requester_slack: str, lead_ids: list[int]) -> None:
    """Remember that this exact lead set was priced, free, for this rep."""
    import time

    _PRICED_RUNS[_priced_run_key(requester_slack, lead_ids)] = time.monotonic()


def _require_priced_run(requester_slack: str, lead_ids: list[int]) -> bool:
    """Whether this exact lead set was priced recently enough to be confirmed."""
    import time

    priced = _PRICED_RUNS.get(_priced_run_key(requester_slack, lead_ids))
    return priced is not None and (time.monotonic() - priced) <= PRICED_RUN_TTL_S
...
def _zoominfo_fill_many(
    ...
    if confirm and not _require_priced_run(requester_slack, lead_ids):
        return (
    ...
    if not confirm:
        _record_priced_run(requester_slack, lead_ids)
        return (
