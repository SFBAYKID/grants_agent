...
def find_contact(
    ...
    if outcome.status == "needs_operator_retry":
        # This fell through to the "I checked everything" sentence below, which was
        # false twice over: nothing was checked (the finder was never called) and
        # nothing was logged (no not_found row exists). It fires on exactly the
        # leads an earlier outage already burned.
        return (
            "I did NOT check this one. An earlier contact lookup for it ended "
            "without proof of whether it was billed, so I refuse to run it again "
            "until someone clears that. This is not 'no contact found' — nothing "
            f"has been established about them either way.{org_line}"
        )
    return (
...
# The bulk-pull money gate moved to `paid_pull` at the 1000-line cap; re-exported
# because tests and every dispatch site refer to these by name on this module.
from .paid_pull import (  # noqa: E402
    MAX_CREDITS_PER_CALL as MAX_CREDITS_PER_CALL,
    PRICED_RUN_TTL_S as PRICED_RUN_TTL_S,
    _PRICED_RUNS as _PRICED_RUNS,
    _priced_run_key as _priced_run_key,
    _record_priced_run as _record_priced_run,
    _require_priced_run as _require_priced_run,
    _zoominfo_fill_many as _zoominfo_fill_many,
)
