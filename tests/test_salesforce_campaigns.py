...
class FakeGateway:
    ...
    users_by_email: dict[str, list[campaigns.SalesforceRecordRef]] = field(
        default_factory=lambda: {
            "chase@monarchconnected.com": [
                campaigns.SalesforceRecordRef(
                    "User",
                    USER_ID,
                    "Chase Gonzales",
                    campaigns_link("User", USER_ID),
                )
            ]
        }
    )
    ...
    def get_record(self, sobject: str, record_id: str) -> campaigns.SalesforceRecordRef:
        ...
        return campaigns.SalesforceRecordRef(
            ...
            record_id,
            "Known Person",
            campaigns_link(sobject, record_id),
    ...
    def find_active_user_by_email(
        ...
        """Resolve exact configured fake Salesforce users by rep email."""
...
def test_campaign_create_timeout_is_unknown_and_not_retried(tmp_path: Path) -> None:
    ...
    class TimeoutGateway(FakeGateway):
        ...
        def create_campaign(
            ...
            """Provide test-local behavior for create campaign."""
            self.calls.append("create_campaign")
...
def test_org_only_lead_preview_contains_no_person_fields(tmp_path: Path) -> None:
    ...
    assert proposed["OwnerId"] == USER_ID
    for forbidden in ("FirstName", "Email", "Title", "Phone"):
        assert forbidden not in proposed
    assert "no individual contact" in proposed["Description"].lower()
    # The Description now also says WHAT THE RECORD IS and what to do next, because a
    # Lead reading only "organization-only" gave a rep nothing to act on.
    assert "next step" in proposed["Description"].lower()
    # ORGANISATION facts are not person facts and belong here even with no contact.
    # These twelve Leads reached the California campaign with an empty address.
    assert proposed.get("State") == "CA"
    assert proposed.get("Industry") == "K-12 Schools"
    assert "Organization-only" in action.preview
    assert (
        "New Lead owner: Chase Gonzales (chase@monarchconnected.com)" in action.preview
    )


def test_confirmed_org_only_lead_keeps_requester_owner_id(tmp_path: Path) -> None:
    """The immutable preview OwnerId is the value sent to Salesforce on confirmation."""
    conn = db.connect(tmp_path / "t.db")
    grant_id = _lead(conn, "A1", "Alpha School District")
    gateway = FakeGateway()
    action = campaigns.prepare_membership(
    ...
    assert result.state is campaigns.CampaignActionState.COMPLETE
    assert len(gateway.created_lead_payloads) == 1
    assert gateway.created_lead_payloads[0]["OwnerId"] == USER_ID


def test_org_only_lead_refuses_unmapped_slack_requester(tmp_path: Path) -> None:
    """An unapproved Slack identity cannot fall back to the integration user."""
    conn = db.connect(tmp_path / "t.db")
    grant_id = _lead(conn, "A1", "Alpha School District")
    gateway = FakeGateway()
...
def test_org_only_lead_refuses_missing_salesforce_rep(tmp_path: Path) -> None:
    """A rostered rep without an active Salesforce user must be fixed first."""
    conn = db.connect(tmp_path / "t.db")
    grant_id = _lead(conn, "A1", "Alpha School District")
    gateway = FakeGateway(users_by_email={})
    with pytest.raises(ValueError, match="No active Salesforce user matches"):
        campaigns.prepare_membership(
            conn,
            gateway,
            "TWORK",
...
def test_org_only_lead_refuses_ambiguous_salesforce_rep(tmp_path: Path) -> None:
    """Duplicate active-user matches cannot be silently assigned."""
    conn = db.connect(tmp_path / "t.db")
    grant_id = _lead(conn, "A1", "Alpha School District")
    gateway = FakeGateway(
        users_by_email={
            "chase@monarchconnected.com": [
                ...
                campaigns.SalesforceRecordRef(
                    ...
                    "005000000000002",
                    "Chase Two",
                    campaigns_link("User", "005000000000002"),
                ),
            ]
        }
    )
    with pytest.raises(ValueError, match="Multiple active Salesforce users match"):
        campaigns.prepare_membership(
            conn,
            gateway,
            "TWORK",
...
def test_membership_skips_duplicates_and_reports_partial(tmp_path: Path) -> None:
    ...
    beta_ref = campaigns.SalesforceRecordRef(
        ...
        CONTACT_ID,
        "Beta Contact",
        campaigns_link("Contact", CONTACT_ID),
        company="Beta School District",
        state="CA",
    )
    gateway = FakeGateway(
        people={
...
def test_action_hard_cap_is_200(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="between 1 and 200"):
        campaigns.prepare_membership(
            ...
            "123.4",
            "UCHASE",
            FakeGateway().get_record("Campaign", CAMPAIGN_ID),
            list(range(1, 202)),
        )
