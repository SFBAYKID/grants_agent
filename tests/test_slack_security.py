...
class MembershipClient:
    ...
    def __init__(
        self, *, deleted: bool = False, bot: bool = False, fail: bool = False
    ) -> None:
    ...
    def conversations_members(self, **_kwargs: object) -> dict[str, object]:
        ...
        return {"members": ["UCHASE"], "response_metadata": {"next_cursor": ""}}
...
def test_deleted_bot_or_slack_outage_fails_closed() -> None:
    ...
    assert (
        grant._active_human_channel_member(
            MembershipClient(deleted=True), "UCHASE", "CGRANTS"
        )
        is False
    )
    assert (
        grant._active_human_channel_member(
            MembershipClient(bot=True), "UCHASE", "CGRANTS"
        )
        is False
    )
    assert (
        grant._active_human_channel_member(
            MembershipClient(fail=True), "UCHASE", "CGRANTS"
        )
        is False
    )
