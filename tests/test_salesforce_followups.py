...
class FakeSlack:
    ...
    def __init__(self, fail: bool = False) -> None:
        """Optionally simulate an ambiguous transport failure on every post."""
        self.fail = fail
...
def _eligible(tmp_path: Path) -> tuple[Any, followups.FollowupCandidate]:
    ...
    conn.execute(
        """INSERT INTO leads(id,source,source_item_id,entity_name,first_seen,last_seen)
           VALUES (1,'test','1','MT ADAMS SCHOOL DISTRICT',?,?)""",
        (joined, joined),
    )
    conn.execute(
    ...
    proposed = json.dumps(
        {"salesforce_ref": None, "proposed_lead": {"Company": "Mt Adams"}}
    )
    conn.execute(
        """INSERT INTO crm_action_items
               (action_id,lead_id,canonical_entity_key,operation,proposed_json,state,
                salesforce_id,campaign_member_id,verification_state)
               VALUES ('members',1,'mt-adams','create_org_lead',?,'added',
                       '00QTEST','00vTEST','verified')""",
        (proposed,),
    )
    conn.commit()
...
def _reader(
    ...
    *,
    ...
    def fake(query: str) -> tuple[list[dict[str, object]], str]:
        """Route one SOQL query to its canned sobject result."""
        if fail:
        ...
        if "FROM Lead" in query:
            return (
                [{"Id": "00QTEST", "LastActivityDate": last_activity}],
                "https://sf",
            )
...
def test_open_task_future_event_and_old_task_do_not_suppress(
...
def test_last_activity_and_response_conservatively_suppress(
    monkeypatch: Any, tmp_path: Path
) -> None:
...
def test_reader_failure_is_unknown_and_posts_nothing(
    monkeypatch: Any, tmp_path: Path
) -> None:
    ...
    assert (
        conn.execute("SELECT state FROM salesforce_followup_state").fetchone()[0]
        == "unknown"
    )


def test_dry_run_has_zero_writes_and_clean_copy(
    monkeypatch: Any, tmp_path: Path
) -> None:
    ...
    assert (
        conn.execute("SELECT COUNT(*) FROM salesforce_followup_state").fetchone()[0]
        == 0
    )
...
def test_delivery_is_one_shot_and_timeout_never_retries(
    monkeypatch: Any, tmp_path: Path
) -> None:
...
def test_delivered_reminder_registers_natural_reply_thread(
    monkeypatch: Any, tmp_path: Path
) -> None:
...
def test_followup_claims_atomic_rich_campaign_slot(
    ...
    assert (
        pacing.reserve_daily_slot(conn, "C", now, "rich_award", "competing-rich")
        is False
    )
