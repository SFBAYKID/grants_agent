...
def test_fresh_database_reaches_v26_with_all_rich_tables(tmp_path: Path) -> None:
    """A brand-new database applies every migration through 26."""
    conn = db.connect(tmp_path / "fresh.db")
    assert (
        conn.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0] == 26
    )
    for table in (
        "rich_card_snapshots",
        "rich_card_actions",
...
def test_upgrade_from_each_supported_historical_schema(
    ...
    """Every ledger version that existed upgrades to v26 with integrity intact."""
