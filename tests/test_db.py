...
def _lead(item_id: str = "A1", source: str = "usaspending:16.071") -> Lead:
    """Provide test-local behavior for lead."""
    return Lead(
        item=RawItem(
            source=source,
            item_id=item_id,
            title="SVPP award",
            entity="Castle Rock SD 401",
            state="WA",
            program="SVPP",
            amount=500_000.0,
            start="2025-10-01",
...
def test_upsert_dedups_on_source_and_item_id(tmp_path: Path) -> None:
    """Verify upsert dedups on source and item id."""
...
def test_upsert_refreshes_last_seen(tmp_path: Path) -> None:
    """Verify upsert refreshes last seen."""
    conn = db.connect(tmp_path / "t.db")
...
def test_versioned_migrations_and_backfill_suppression(tmp_path: Path) -> None:
    ...
    # 14-21 add the rich award-card campaign schema (forward-only after 13).
    assert versions == [1, 2, 3, 4, 5, 6, 7, 8, 9, 13, 14, 15, 16, 17, 18, 19, 20, 21]
    crm_tables = {
        row[0]
        for row in conn.execute(
...
def test_failed_migration_rolls_back_schema_and_version(
    ...
    assert (
        raw.execute("SELECT 1 FROM schema_migrations WHERE version=999").fetchone()
        is None
    )
