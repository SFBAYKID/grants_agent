"""Drip-engine tests: window math, pacing/slot gates, engagement dedupe and
territory tagging — i.e. WHEN Grant may post. The message builders live in
test_drip_builders.py (split for CLAUDE.md rule 4). All offline; the LLM layer is not
exercised here (its failure mode is tested by contract: bad output degrades to an honest
"didn't parse").
"""
...
def test_backfilled_gold_award_still_reaches_the_daily_card(tmp_path: Path) -> None:
    ...
    assert (
        conn.execute("SELECT suppressed FROM funding_events").fetchone()["suppressed"]
        == 1
    ), "fixture must reproduce the suppressed shape"
...
def test_gold_award_outranks_an_open_rfp_even_when_backfilled(tmp_path: Path) -> None:
    ...
    conn = db.connect(tmp_path / "t.db")
    mk_rfp(conn, iid="R_LOSE", end="2031-12-31")
    mk_lead(
        conn,
        iid="G_WIN",
...
def test_sibling_rfps_do_not_render_as_the_same_card(tmp_path: Path) -> None:
    ...
    mk_rfp(
        conn,
        iid="HVAC",
        entity="Pennsylvania Department of Corrections",
...
def test_cap_holds_when_recording_a_confirmed_send_fails(
    ...
    try:
        ...
        later = datetime(
            2026, 7, 22, 21, 0, tzinfo=timezone.utc
        )  # 2:00 PM PT, same day
