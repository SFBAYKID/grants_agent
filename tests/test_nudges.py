...
def test_a_nudge_respects_the_mentioned_reps_own_clock(tmp_path: Path) -> None:
    """The shared window is 7:00 Eastern to 17:00 Pacific — 20:00 Eastern.

    Correct for a channel card that pings nobody, wrong for a message that
    @-mentions one person. Measured on 2026-08-10: a send judged fine at 20:23
    Pacific was 23:23 for Kerry, who is in America/New_York, and the message was an
    apology to the one rep who had already disengaged.
    """
    conn = _conn(tmp_path)
    _expired_preview(conn, NOW - timedelta(hours=6))
    conn.execute("UPDATE crm_actions SET requested_by='U01E908206M' WHERE id='act-1'")
    conn.commit()
    candidate = nudges.candidates(conn, NOW)[0]
    assert candidate.target_slack == "U01E908206M"

    # 03:23 UTC on a Monday = 23:23 Sunday for her, and 20:23 Sunday Pacific.
    late = datetime(2026, 8, 10, 3, 23, tzinfo=timezone.utc)
    assert "working hours" in nudges.pacing_reason(conn, candidate, late, force=True), (
...
def test_a_rep_with_no_recorded_timezone_behaves_exactly_as_before(
    tmp_path: Path,
) -> None:
    """An absent zone must never make Grant quieter OR noisier than the old rule.

    Guessing one from a state or an area code would be worse than none: it would look
    authoritative while being wrong for anyone who moved.
    """
    conn = _conn(tmp_path)
    _expired_preview(conn, NOW - timedelta(hours=6))  # requested_by = U0REP, no zone
    candidate = nudges.candidates(conn, NOW)[0]
    late = datetime(2026, 8, 10, 3, 23, tzinfo=timezone.utc)
    assert "working hours" not in nudges.pacing_reason(
