...
def test_audit_bundle_is_one_fixed_all_or_none_transaction(
    ...
    result = gateway.create_lead_audit_bundle(
        ...
        ACTION_ID,
        "Verified sources",
        "No customer outreach. Action " + ACTION_ID,
        "2026-07-15",
    )
    ...
    assert not any(
        "Lead/" in item["url"]
        or "Campaign" in item["url"]
        or "Opportunity" in item["url"]
...
def test_existing_lead_enrichment_is_one_four_part_transaction(
    ...
    assert items[0] == {
        ...
        "body": {"Website": "https://district.test"},
    }
...
def test_composite_rollback_preserves_exact_subrequest_error(
    ...
    assert (
        "INVALID_FIELD" in result.error and "grantLeadUpdate HTTP 400" in result.error
    )
...
def test_audit_readback_compares_note_link_task_and_truthful_copy(
    ...
    task_body = (
        "Grant updated Website. No customer outreach was performed. Action " + ACTION_ID
    )

    def fake_get(path: str, _params: dict[str, str]) -> dict[str, object]:
        if path.startswith("sobjects/ContentNote/"):
            return {
                "Title": gateway_mod._research_note_title(note_body, ACTION_ID),
...
def test_research_note_title_leads_with_organization_name() -> None:
    ...
    title = gateway_mod._research_note_title(
        "Grant research summary for Birmingham Community Charter High School\n\n"
        "Funding record: https://source.test",
        ACTION_ID,
    )
    assert title.startswith(
        "Grant research — Birmingham Community Charter High School — "
    )
...
def test_audit_readback_follows_salesforce_content_note_url(
    ...
    monkeypatch.setattr(
        gateway_mod.requests, "get", lambda *_args, **_kwargs: RawResponse()
    )
    result = gateway_mod.LeadAuditResult(
        True, "069000000000001", "06A000000000001", "00T000000000001"
    )
