...
def _lead(conn: sqlite3.Connection) -> int:
    ...
    db.upsert_lead(conn, Lead(RawItem(
        "usaspending:16.071", "A1", "SVPP award", "Castle Rock School District",
        "WA", "SVPP", 500_000.0, "2026-06-01", "2028-09-30", "", {},
        event_type=FundingEventType.AWARD_OBLIGATED, event_date="2026-06-15",
        date_precision=DatePrecision.DAY,
        verification_status=VerificationStatus.VERIFIED), LeadGrade.GOLD))
...
def _found() -> salesforce.SFResult:
    ...
    return salesforce.SFResult(status=salesforce.SFResultStatus.FOUND, matches=[
        salesforce.SFMatch(
            "Account", "001A", "Castle Rock School District", "", "Anthony",
            "https://sf.test/lightning/r/Account/001A/view", "high", state="WA"),
        salesforce.SFMatch(
            "Opportunity", "006A", "Security Upgrade", "", "Anthony",
            "https://sf.test/lightning/r/Opportunity/006A/view", "high",
            account_id="001A", stage="Prospecting", is_closed=False),
    ])
...
def test_sync_persists_read_only_account_and_opportunity(
    ...
    state = conn.execute(
        "SELECT * FROM salesforce_lookup_state WHERE lead_id=?", (lead_id,)).fetchone()
    matches = conn.execute(
        "SELECT * FROM salesforce_matches WHERE lead_id=?", (lead_id,)).fetchall()
...
def test_sync_dry_run_writes_nothing(
    ...
    assert conn.execute("SELECT COUNT(*) FROM salesforce_lookup_state").fetchone()[0] == 0
...
def test_outage_preserves_last_known_matches(
    ...
    conn.execute("UPDATE salesforce_lookup_state SET checked_at='2000-01-01T00:00:00+00:00'")
    conn.commit()
    ...
    assert conn.execute(
        "SELECT status FROM salesforce_lookup_state").fetchone()["status"] == "unavailable"
