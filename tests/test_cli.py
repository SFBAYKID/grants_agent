...
def _item() -> RawItem:
    ...
    return RawItem(
        source="test", item_id="1", title="test", entity="Test District",
        state="CA", program="", amount=None, start="", end="", url="", raw={},
    )
...
def test_drip_dry_run_uses_readonly_database(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_drip(client: object, channel: str, conn: object,
...
def test_outreach_retry_dry_run_uses_readonly_database(
    ...
    def fake_retry(conn: object, dry_run: bool = False
...
def test_salesforce_sync_dry_run_uses_readonly_database(
    ...
    def fake_sync(conn: object, limit: int = 25,
                  dry_run: bool = False) -> salesforce_sync.SyncSummary:
...
def test_slack_failure_cli_lists_and_reviews_without_replay(
    ...
    cli.db.finish_slack_event(
        conn, "evt-1", error="delivery failed",
