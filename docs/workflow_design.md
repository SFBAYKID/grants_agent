...
- Organization-only Lead ownership is **verified live** in `monarchdev` on 2026-07-16: one synthetic
  Lead was created and read back with Chase's exact active Salesforce owner/email, blank person
  fields, and one unique marker match. The sandbox record intentionally remains because the writer
  has no delete permission. Campaign creation/member execution and production remain needs-testing.
- Campaign intake is the only write exception: separate credentials, feature flag off,
  ...
  Members; Account/Contact/Opportunity records are never modified. Each new organization-only Lead
  uses the requesting rep's exact active Salesforce User `OwnerId`, resolved from the rep roster
  email. An unmapped, inactive, missing, or ambiguous owner blocks the preview instead of assigning
  the integration user and requiring a later manual transfer.
